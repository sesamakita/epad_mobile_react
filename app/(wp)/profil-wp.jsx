...
import { ROLE_COLORS } from '../../src/constants/roles';

export default function ProfilWP() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { user, logout } = useAuthStore();
  const rc = ROLE_COLORS[user?.role] || Colors.info[500];

  return (
    <View style={[s.c, { paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        <View style={s.hdr}><Text style={s.t}>Profil Saya</Text></View>
        <View style={s.pCard}>
          ...
          <View style={[s.badge, { backgroundColor: rc + '15' }]}>
            <Text style={[s.badgeT, { color: rc }]}>Wajib Pajak</Text>
          </View>
          {user?.npwpd && <Text style={s.meta}>NPWPD: {user.npwpd}</Text>}
          {user?.nik && <Text style={s.meta}>NIK: {user.nik}</Text>}
          {user?.alamat && <Text style={s.meta}>{user.alamat}</Text>}
